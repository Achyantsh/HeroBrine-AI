...
import { CalendarDays, Clock, MoreHorizontal, CheckCircle2, Trash2, Pencil } from "lucide-react"
...
export function CommitmentCard({ commitment, index, onViewDetails }: CommitmentCardProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.3, delay: index * 0.04, ease: [0.25, 0.1, 0.25, 1] as const }}
      className="group flex items-start gap-4 rounded-xl border border-border/60 bg-card p-4 hover:border-border hover:shadow-sm transition-all cursor-pointer"
    ...
    >
      ...
      <div
        className={cn(
          ...
          commitment.status === "missed" && "bg-red-500",
      ...
      <div className="flex-1 min-w-0 space-y-1.5">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Badge
            variant="outline"
            className={cn("text-[10px] px-1.5 py-0 h-4 capitalize border-0", categoryColors[commitment.category])}
          >
            {commitment.category}
          </Badge>
          <Badge
